...
function readData() {
	return JSON.parse(fs.readFileSync(path.resolve(__dirname, `../${PUBLIC_DIR}/data.json`)))
}

function writeData(dataToWrite) {
	fs.writeFileSync(path.resolve(__dirname, `../${PUBLIC_DIR}/data.json`), JSON.stringify(dataToWrite, null, 4))
}
