...
const compiler = webpack(backendWebpackConfig); //move your `devServer` config from `webpack.config.js`
...
app.post('/login', (req, res) => {
	...
	if (login === AUTH_DATA.login) {
		if (password === AUTH_DATA.password) {
			...
			res.end(response(null, 'Неправильный пароль'))
		}
		return
	}

	res.end(response(null, 'Неправильный логин'))
})
